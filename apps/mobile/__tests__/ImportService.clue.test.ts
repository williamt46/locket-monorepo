...
describe('ImportService - Clue', () => {
    ...
    it('handles spotting as non-period with flow 0', () => {
        const result = parseClueExport(clueSample as unknown as ClueExport);
        const jan4 = result.entries.find(e => e.ts === Date.UTC(2024, 0, 4));
    ...
    it('sets isStart and isEnd flags on consecutive runs', () => {
        ...
        const jan30 = result.entries.find(e => e.ts === Date.UTC(2024, 0, 30));
    ...
    it('preserves bbt values natively', () => {
        const result = parseClueExport(clueSample as unknown as ClueExport);
        const jan20 = result.entries.find(e => e.ts === Date.UTC(2024, 0, 20));

        expect(jan20?.bbt).toBe(98.6);
        expect(jan20?.isPeriod).toBe(false); // No period tags
    });

    it('preserves unmapped keys defensively', () => {
        const result = parseClueExport(clueSample as unknown as ClueExport);

        const jan1 = result.entries.find(e => e.ts === Date.UTC(2024, 0, 1));
        expect(jan1?.unmapped?.['pain/headache']).toBe(true);

        const jan2 = result.entries.find(e => e.ts === Date.UTC(2024, 0, 2));
        expect(jan2?.unmapped?.['pain/cramps']).toBe(true);
        expect(jan2?.unmapped?.['feelings/sad']).toBe(true);

        // Warning generated for unmapped keys
        expect(result.warnings.length).toBeGreaterThan(0);
        expect(result.warnings[0]).toContain("unmapped keys");
    });
    ...
    it('gaps produce no interpolated entries', () => {
        ...
        const jan10 = result.entries.find(e => e.ts === Date.UTC(2024, 0, 10));
