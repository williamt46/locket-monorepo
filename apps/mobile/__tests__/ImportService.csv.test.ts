...
describe('ImportService - CSV Parsing', () => {
    ...
    it('preserves bbt and note values natively', () => {
        ...
        expect(jan1?.note).toBe('first day of year');
    ...
    it('preserves unmapped columns defensively', () => {
        const result = parseCsvExport(isoCsv);
        const jan1 = result.entries.find(e => e.ts === Date.UTC(2024, 0, 1));

        // 'mood' is an unrecognized column, should be stashed
        expect(jan1?.unmapped?.['mood']).toBe('sad');
    });
    ...
    it('ambiguous date format emits warning', () => {
        ...
        const may6 = result.entries.find(e => e.ts === Date.UTC(2024, 4, 6));
